...
- [Orval hook options](orval-query-options.md) — generated react-query hooks: passing `query` options requires explicit `queryKey` via exported getXQueryKey helpers.
...
- [Monorepo env gotchas](monorepo-env-gotchas.md) — pnpm install after copy; zod coerce.boolean("false") is true; run codegen ONLY via @workspace/api-spec codegen script (zod/v4 patch).
